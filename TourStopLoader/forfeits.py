...
def delete_game(tournament_id, bracket_id, player_id, round_number):
    db = 'masters_cups'
    sql = "DELETE FROM %(db)s.games WHERE tournament_id = '%(tournament_id)s' and bracket_id = '%(bracket_id)s' and player_id = '%(player_id)s' and round_number = %(round_number)s"
    print(sql % locals())
...
def get_match_info(tournament_id, bracket_id, match):
    ...
    if 'readyAt' not in match['top']:
        ...
        print('P1', tournament_id, bracket_id, tmp_name1, tmp_name2, round_num)
        delete_game(tournament_id, bracket_id, p1_id, round_num)
        delete_game(tournament_id, bracket_id, p2_id, round_num)
        return
    if 'readyAt' not in match['bottom']:
        tmp_name1 = player_tourn_names.get((tournament_id, p1_id), p1_id)
        tmp_name2 = player_tourn_names.get((tournament_id, p2_id), p2_id)
        print('P2', tournament_id, bracket_id, tmp_name1, tmp_name2, round_num)
        delete_game(tournament_id, bracket_id, p1_id, round_num)
        delete_game(tournament_id, bracket_id, p2_id, round_num)
        return
    if True:
...
for tournament_id, tournament_time, tournament_region, tournament_name in get_masters_cups(end_date):
    ...
    if len(stage_ids) < 2: 
        ...
        continue
    #create_tournament(tournament_id, tournament_name, tournament_time, stage_ids[0], stage_ids[1])
