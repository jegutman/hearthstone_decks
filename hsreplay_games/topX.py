...
if len(start_date) < 3:
    start_date = (datetime.now() - timedelta(days=int(start_date))).strftime("%Y_%m_%d")
region = '%'
if len(sys.argv) > 3:
    region = sys.argv[3]
print(start_date)
...
for game_id, date, time, region, p1, p2, p1_rank, p2_rank, archetype1, archetype2, num_turns, result, p1_deck_code, p2_deck_code, first in cursor.fetchall():
    ...
    print("%22s %10s %s    %2s %-25s %-5s %-25s %-5s %-5s %-25s %-25s %2s %s" % (game_id, date, time_string, region, p1, result, p2, p1_rank, p2_rank, archetype1, archetype2, num_turns, first))
