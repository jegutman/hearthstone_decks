...
lineups_to_test = [josh_test_1]#, level_3]

for lineup in lineups:
    for lu_test in lineups_to_test:
        win_rates_against_good[lineup] = win_rates_against_good.get(lineup, []) + [round(win_rate(list(lineup), lu_test, win_pcts), 3)]

for i,j in sorted(win_rates_against_good.items(), key=lambda x:sum(x[1]))[-10:]:
    print "%-90s %s %s" % (i,j, round(sum(j)/len(j),3))
