...
def process_deck(deck_code, deck_class, name, archetype):
    db, table = 'deckstrings,decks'.split(',')

    deck_code      = deck_code
    time           = datetime.datetime.now().strftime('%s')
    date           = datetime.datetime.now().strftime('%Y_%m_%d')
    server         = 'DHATX Playoffs Loader'
    user           = 'DHATX Playoffs Loader'
    is_private     = 0
